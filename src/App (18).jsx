  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui"}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(380px,95vw)",padding:26}}>
        ...
        <div style={{fontSize:13,color:C.muted,marginBottom:18}}>How many videos for {monthLabel(month)}?</div>
        <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:16}}>
          {[4,6,8,10,12,15,20].map(n=>(
            <button key={n} onClick={()=>setVal(n)} style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${val===n?C.accent:C.border}`,background:val===n?C.accent+"18":C.surface,color:val===n?C.accent:C.text,fontSize:14,fontWeight:val===n?700:400,cursor:"pointer"}}>{n}</button>
          ))}
        </div>
        <div style={{marginBottom:18}}><div style={{fontSize:11,color:C.muted,marginBottom:5}}>Custom</div><input type="number" min="1" max="60" value={val} onChange={x=>setVal(+x.target.value)} style={{...inp,width:90,textAlign:"center"}}/></div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui"}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:shMd,width:"min(380px,95vw)",padding:26}}>
        ...
        {[["Video title","title","text"],["Target date","targetDate","date"]].map(([l,k,t])=>(
          <div key={k} style={{marginBottom:12}}><div style={{fontSize:11,color:C.muted,marginBottom:4,fontWeight:500}}>{l}</div><input type={t} value={f[k]} onChange={x=>set(k,x.target.value)} onKeyDown={x=>x.key==="Enter"&&save()} style={inp} placeholder={k==="title"?"What's this video about?":""}/></div>
        ))}
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:9}}>
        {STAGES.map(stage=>{
          ...
          return(
            <div key={stage.id} style={{background:C.light,borderRadius:10,padding:9}}>
              ...
              {cards.map(v=>{
                ...
                const unlocked=isPub&&daysSince(v.publishDate||v.createdAt)>=7;
                const hasMet=v.metricsAdded;
                const isCf=confirmDel===v.id;
                return(
                  <div key={v.id} style={{background:C.surface,borderRadius:8,padding:10,marginBottom:7,border:`1px solid ${hasMet?"#BBF7D0":C.border}`,borderLeft:`3px solid ${stage.color}`}}>
                    <div style={{fontSize:11,fontWeight:700,color:C.text,marginBottom:4,lineHeight:1.3}}>{v.title}</div>
                    <div style={{fontSize:10,color:C.muted,marginBottom:5,display:"flex",gap:5,flexWrap:"wrap"}}><span>{PLT[v.platform]||"📱"} {v.platform}</span>{v.targetDate&&<span>📅 {v.targetDate}</span>}</div>
                    {isPub&&!hasMet&&<div style={{marginBottom:5}}>{unlocked?<span style={{fontSize:9,fontWeight:700,background:"#DCFCE7",color:C.green,padding:"2px 6px",borderRadius:20}}>✅ Ready for metrics</span>:<span style={{fontSize:9,color:C.muted}}>🔒 {Math.max(0,7-daysSince(v.publishDate||v.createdAt))}d left</span>}</div>}
                    {hasMet&&<span style={{fontSize:9,fontWeight:700,background:"#DCFCE7",color:C.green,padding:"2px 6px",borderRadius:20,marginBottom:5,display:"inline-block"}}>📊 Metrics done</span>}
                    <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:3}}>
                      {stage.id!=="published"&&<button onClick={()=>onMoveVideo(v.id,STAGES[STAGES.findIndex(s=>s.id===stage.id)+1]?.id)} style={{fontSize:9,padding:"2px 7px",background:C.light,border:`1px solid ${C.border}`,borderRadius:4,cursor:"pointer",color:C.text}}>→ {STAGES[STAGES.findIndex(s=>s.id===stage.id)+1]?.label}</button>}
                      {isPub&&!hasMet&&unlocked&&<button onClick={()=>setMetricsVid(v)} style={{fontSize:9,padding:"2px 7px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:4,cursor:"pointer",color:C.green,fontWeight:700}}>📊 Add metrics</button>}
                      {!isCf&&<button onClick={()=>setConfirmDel(v.id)} style={{fontSize:9,padding:"2px 7px",background:"#FEE2E2",border:"1px solid #FECACA",borderRadius:4,cursor:"pointer",color:C.red}}>🗑</button>}
                      {isCf&&<div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:9,color:C.red,fontWeight:700}}>Delete?</span><button onClick={()=>{onDeleteVideo(v.id);setConfirmDel(null);}} style={{fontSize:9,padding:"1px 6px",background:C.red,border:"none",borderRadius:3,cursor:"pointer",color:"#fff",fontWeight:700}}>Yes</button><button onClick={()=>setConfirmDel(null)} style={{fontSize:9,padding:"1px 6px",background:C.light,border:`1px solid ${C.border}`,borderRadius:3,cursor:"pointer",color:C.text}}>No</button></div>}
                    </div>
                  </div>
                );
              })}
