            ...
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}>
              ...
              {m.role==="assistant"&&(
                <div style={{display:"flex",gap:8,marginTop:5,marginLeft:2}}>
                  ...
                  <button onClick={()=>onSaveShotList(m.content)}
                    style={{display:"block",width:"100%",maxWidth:320,padding:"10px 0",background:"#F59E0B",color:"#FFF",border:"none",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:700,marginBottom:6}}>
                    ✓ Save this shot list to my card
                  </button>
                </div>
              )}
            </div>
          ...
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}>
              ...
              {m.role==="assistant"&&i===msgs.length-1&&(
                <div style={{marginTop:10,marginLeft:2,maxWidth:320}}>
                  <button onClick={()=>onSaveScript&&onSaveScript(m.content)}
                    style={{display:"block",width:"100%",padding:"11px 0",background:C.green,color:"#FFF",border:"none",borderRadius:10,cursor:"pointer",fontSize:13,fontWeight:700,marginBottom:6,boxShadow:"0 2px 8px rgba(5,150,105,.3)"}}>
                    ✓ Save this script to my card
                  </button>
                  <div style={{display:"flex",gap:12}}>
                    <button onClick={()=>copyText(m.content)} style={{fontSize:11,color:C.muted,background:"none",border:"none",cursor:"pointer",padding:0}}>Copy text ↗</button>
                    <button onClick={()=>setInput("Make the hook more punchy")} style={{fontSize:11,color:C.accent,background:"none",border:"none",cursor:"pointer",padding:0}}>Refine it →</button>
                  </div>
                </div>
              )}
            </div>
