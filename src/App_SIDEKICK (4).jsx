...
const TOUR_STEPS = [
  {
    step: 1,
    emoji: "🎯",
    title: "Set your video goal",
    body: "Before anything else — decide how many videos you want to publish this month. This number becomes your north star. Everything in the app tracks progress toward it.",
    action: "Set my goal →",
    arrow: "top-right",
    highlight: "goal",
  },
  {
    step: 2,
    emoji: "➕",
    title: "Add your first video idea",
    body: "Got a video idea? Add it here. Just give it a title, pick the platform, and set a target date. That's it — your idea now lives in the pipeline.",
    action: "Got it →",
    arrow: "top-right",
    highlight: "add",
  },
  {
    ...
    body: "As you work on it, drag the card forward. Idea → Production → Editing → Published. Each move keeps you accountable and shows your progress at a glance.",
    action: "Makes sense →",
    arrow: "bottom-center",
    highlight: "stages",
  },
  {
    ...
    body: "When your video is live, move the card to Published. ContentHub Sidekick doesn't post for you — you publish it yourself on TikTok, Instagram, or wherever. Then come back here and mark it done.",
    action: "Understood →",
    arrow: "bottom-right",
    highlight: "publish",
  },
  {
    step: 5,
    emoji: "📊",
    title: "Come back in 7 days for metrics",
    body: "After 7 days, your video will have real data — views, engagement, what worked. Come back, click 'Add metrics', fill in the numbers, and the app tells you exactly what's driving your growth.",
    action: "Let's go! 🚀",
    arrow: "bottom-right",
    highlight: "metrics",
  },
...
function ProductTour({onFinish,onSetGoal}){
  const[step,setStep]=useState(0);
  const current=TOUR_STEPS[step];
  const isLast=step===TOUR_STEPS.length-1;

  const arrowPos={
    "top-right":{top:80,right:120},
    "bottom-center":{top:"50%",left:"50%",transform:"translate(-50%,-50%)"},
    "bottom-right":{top:"55%",right:60},
  };
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:9999,fontFamily:"system-ui,sans-serif"}}>
      {/* Dark overlay */}
      <div style={{position:"absolute",inset:0,background:"rgba(15,23,42,.75)",backdropFilter:"blur(1px)"}}/>

      {/* Step indicator */}
      <div style={{position:"absolute",top:20,left:"50%",transform:"translateX(-50%)",display:"flex",gap:6,zIndex:1}}>
        {TOUR_STEPS.map((_,i)=>(
          <div key={i} style={{width:i===step?20:6,height:6,borderRadius:20,background:i===step?"#FFF":i<step?"#93C5FD":"rgba(255,255,255,.3)",transition:"all .2s"}}/>
        ))}
      </div>

      {/* Arrow indicator pointing to relevant area */}
      <div style={{position:"absolute",...arrowPos[current.arrow||"bottom-center"],fontSize:28,color:"#93C5FD",zIndex:1,pointerEvents:"none",animation:"bounce 1s infinite"}}>
        {arrowDir[current.arrow||"bottom-center"]}
      </div>

      {/* Tooltip card */}
      <div style={{position:"absolute",bottom:60,left:"50%",transform:"translateX(-50%)",width:"min(420px,90vw)",background:C.surface,borderRadius:20,padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.4)",zIndex:2}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:28}}>{current.emoji}</span>
            <div>
              <div style={{fontSize:11,color:C.muted,fontWeight:600,letterSpacing:0.5}}>STEP {current.step} OF {TOUR_STEPS.length}</div>
              <div style={{fontSize:18,fontWeight:800,color:C.text,letterSpacing:-0.3}}>{current.title}</div>
            </div>
          </div>
          <button onClick={onFinish} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer",padding:0,lineHeight:1}}>×</button>
        </div>
        <div style={{fontSize:14,color:C.muted,lineHeight:1.7,marginBottom:20}}>{current.body}</div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <button onClick={onFinish} style={{background:"none",border:"none",color:C.muted,fontSize:12,cursor:"pointer",padding:0}}>Skip tour</button>
          <button onClick={next} style={{padding:"10px 22px",background:C.text,color:"#FFF",border:"none",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer"}}>
            {isLast?"Let's go! 🚀":current.action}
          </button>
        </div>
      </div>
...
function CreatorApp({user,profile,onLogout}){
  ...
  const load=useCallback(async()=>{
    ...
    if(isNew) setTimeout(()=>setShowTour(true), 600);
    setLoading(false);
  },[user.id]);
  ...
  const finishTour=()=>{localStorage.setItem(`sk_tour_done_${user.id}`,"1");setShowTour(false);};
  const tourSetGoal=()=>{setShowGoalModal(true);};

  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      {showTour&&<ProductTour onFinish={finishTour} onSetGoal={tourSetGoal}/>}
