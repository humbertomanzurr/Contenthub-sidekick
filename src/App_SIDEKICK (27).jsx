...
function ShootDirector({card,businessProfile,onSaveShotList,onClose}){
  ...
  const buildSystem=()=>{
    ...
    return `You are a video production director helping a small business film a short-form social media video.

BUSINESS: ${bp.businessName||"Unknown"} — ${bp.whatYouDo||""}
PLATFORM: ${card.platform}
VIDEO: "${card.title}"
${card.script?`SCRIPT:\n${card.script}`:"No script saved yet — work with the video title and concept."}

YOUR JOB:
Ask the user about their shoot setup (how many people, names, location, equipment). Then generate a clear numbered shot list they can follow on the day of filming.

SHOT LIST FORMAT:
Each shot should include:
- Shot number and type (e.g. "Shot 1 — Hook (Close-up)")
- Who is in frame and where they should be
- Exactly what they say or do
- Any notes on framing, lighting, or props

Keep it practical for a small team filming with a smartphone. No cinematography jargon. Be specific and actionable.

Start by asking 2-3 quick setup questions before generating the shot list.`;
  };
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 20px 60px rgba(0,0,0,.3)",width:"min(640px,100%)",height:"min(600px,90vh)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
            ...
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}>
              ...
              {m.role==="assistant"&&(
                <div style={{display:"flex",gap:8,marginTop:5,marginLeft:2}}>
                  ...
                  <button onClick={()=>onSaveShotList(m.content)} style={{fontSize:10,color:"#92400E",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:20,cursor:"pointer",padding:"2px 9px",fontWeight:600}}>✓ Save shot list</button>
                </div>
              )}
            </div>
          ))}
        ...
        {started&&(
          <div style={{padding:"12px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0}}>
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} placeholder="2 talents, names are Jake and Maria, filming indoors..." style={{...inp,fontSize:13,padding:"9px 12px"}} disabled={loading}/>
            <button onClick={send} disabled={loading||!input.trim()} style={{padding:"9px 16px",background:loading||!input.trim()?C.border:"#F59E0B",color:loading||!input.trim()?C.muted:"#FFF",border:"none",borderRadius:8,cursor:loading||!input.trim()?"not-allowed":"pointer",fontSize:13,fontWeight:700,flexShrink:0}}>Send</button>
          </div>
        )}
      </div>
    </div>
...
function ShotListViewer({card,businessProfile,onSave,onReplan,onClose}){
  ...
  const printShotList=()=>{
    ...
    <body><h1>Shot List — ${card.title}</h1>
    <div class="meta">${card.platform} · ${businessProfile?.businessName||""} · ${new Date().toLocaleDateString()}</div>
      ...
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 20px 60px rgba(0,0,0,.3)",width:"min(600px,100%)",maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            ...
            <button onClick={()=>setEditing(e=>!e)} style={{fontSize:11,padding:"5px 11px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>{editing?"👁 Preview":"✏️ Edit"}</button>
            <button onClick={printShotList} style={{fontSize:11,padding:"5px 11px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>🖨 Print / PDF</button>
            <button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:20,cursor:"pointer",padding:4}}>×</button>
          </div>
        </div>
        <div style={{flex:1,overflowY:"auto",padding:20}}>
...
GUIDELINES:
- Write scripts that match their brand tone exactly
- For TikTok/Reels/Shorts: aim for 30-60 seconds (150-300 words)
- Always include: Hook (first 3 seconds), Body, CTA
- Format scripts clearly with labeled sections
- Be conversational, ask clarifying questions when needed
- Reference their performance data when suggesting hooks or formats
- Keep it human and authentic to their brand — never generic`;
        ...
        <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}>
              ...
              {m.role==="assistant"&&(
                <div style={{display:"flex",gap:8,marginTop:5,marginLeft:2}}>
                  <button onClick={()=>copyText(m.content)} style={{fontSize:10,color:C.muted,background:"none",border:"none",cursor:"pointer",padding:0}}>Copy ↗</button>
                  <button onClick={()=>onSaveScript&&onSaveScript(m.content)} style={{fontSize:10,color:C.green,background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",padding:"2px 9px",fontWeight:600}}>✓ Save as script</button>
                </div>
              )}
            </div>
          ))}
        ...
        {started&&(
          <div style={{padding:"12px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0}}>
            <input
              ...
              placeholder="Make the hook punchier... / Shorter version... / Write it for Instagram..."
              style={{...inp,fontSize:13,padding:"9px 12px"}}
              disabled={loading}
            />
            <button onClick={send} disabled={loading||!input.trim()} style={{padding:"9px 16px",background:loading||!input.trim()?C.border:C.text,color:loading||!input.trim()?C.muted:"#FFF",border:"none",borderRadius:8,cursor:loading||!input.trim()?"not-allowed":"pointer",fontSize:13,fontWeight:700,flexShrink:0,transition:"all .15s"}}>
              Send
            </button>
          </div>
        )}
  ...
  if(step===-1) return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(480px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"40px 36px",textAlign:"center"}}>
        ...
        <div style={{fontSize:24,fontWeight:900,color:C.text,letterSpacing:-0.5,marginBottom:10,lineHeight:1.2}}>Before we build your dashboard</div>
        <div style={{fontSize:14,color:C.muted,lineHeight:1.7,marginBottom:12,maxWidth:360,margin:"0 auto 12px"}}>
          We need 60 seconds of context about your brand. This powers everything — the pipeline, the analytics, and the AI assistant that helps you plan and script content.
        </div>
        <div style={{fontSize:12,color:C.muted,marginBottom:32,padding:"10px 16px",background:C.light,borderRadius:8,display:"inline-block"}}>
...
function CreatorApp({user,profile,onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {tourStep===2&&<TourBubble step={2} total={6} emoji="➕" title="Add your first video idea" body="Got an idea? Hit the button, give it a title, pick the platform, and set a target date. Your idea now lives in the pipeline." action="Add a video →" onAction={tourAction2} onSkip={finishTour} targetRef={refAdd} side="bottom"/>}
