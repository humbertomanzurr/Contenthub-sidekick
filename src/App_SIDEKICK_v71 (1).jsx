                    ...
                    <div style={{padding:"10px 12px 0",flex:1,minWidth:0,position:"relative"}}>
                      ...
                      {isPub&&!hasMet&&!unlocked&&(
                        <div style={{marginBottom:8,padding:"7px 9px",background:"#F8FAFC",borderRadius:7,border:`1px solid ${C.border}`}}>
                          ...
                          <div style={{fontSize:9,color:C.muted,marginTop:3}}>{daysGone} of 7 days</div>
                        </div>
                      )}
                      {isPub&&!hasMet&&unlocked&&(
                        <div style={{marginBottom:8,padding:"7px 9px",background:"#DCFCE7",borderRadius:7,border:"1px solid #BBF7D0",display:"flex",alignItems:"center",gap:6}}>
    ...
    (async()=>{
      try{
        ...
        if(s?.user){
          ...
          const acct=p?.account_type||cached||"creator";
          if(p&&!p.account_type&&cached){sbUpdate("profiles","id",s.user.id,{account_type:cached});}
          if(!p&&cached){sbInsert("profiles",{id:s.user.id,name:s.user.email,email:s.user.email,account_type:cached});}
          if(acct)localStorage.setItem(`sk_acct_${s.user.id}`,acct);
