...
function ScriptDocument({card,analytics,onSave,onClose}){
  ...
  const[chatMsgs,setChatMsgs]=useState([{role:"ai",text:"What part of this video are you most uncertain about — the hook, the angle, or where to end it?"}]);
  ...
  const dirs=(CHANNEL_DIRECTIONS[platform]||CHANNEL_DIRECTIONS["TikTok"]).slice(0,3);
  ...
  const sendChat=async()=>{
    ...
    setChatLoading(true);
    try{
      const sys=`You are a creative direction assistant for a content creator. You ask questions and give brief creative directions — you never write their script for them. Keep responses under 60 words. Ask one follow-up question at the end. Platform: ${platform}. Video: ${card.title}. Current script: ${JSON.stringify(sections)}.`;
      const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[...chatMsgs.filter(m=>m.role==="user").map(m=>({role:"user",content:m.text})),{role:"user",content:userMsg}],systemPrompt:sys})});
      const d=await r.json();
      if(d.content)setChatMsgs(p=>[...p,{role:"ai",text:d.content}]);
    }catch(e){setChatMsgs(p=>[...p,{role:"ai",text:"Something went wrong — try again."}]);}
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.55)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui",padding:16}}>
      <div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 20px 60px rgba(0,0,0,.25)",width:"min(900px,100%)",maxHeight:"92vh",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* Header */}
        <div style={{padding:"12px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:C.muted}}>{card.title} · {platform}</div>
          </div>
          <button onClick={()=>{
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:"1fr 280px",overflow:"hidden"}}>
          ...
          <div style={{overflowY:"auto",borderRight:`1px solid ${C.border}`}}>
            {sectionDefs.map(sec=>(
              <div key={sec.key} onClick={()=>setFocusField(sec.key)}
                style={{padding:"12px 18px",borderBottom:`0.5px solid ${C.border}`,background:focusField===sec.key?sec.color+"08":"transparent",cursor:"text",transition:"background .15s"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                  <div style={{width:3,height:13,borderRadius:2,background:sec.color}}/>
                  <div style={{fontSize:9,fontWeight:500,color:C.text,textTransform:"uppercase",letterSpacing:1}}>{sec.label}</div>
                ...
                <textarea
                  ...
                  style={{width:"100%",border:"none",outline:"none",background:"transparent",fontSize:13,color:sections[sec.key]?C.text:C.muted,lineHeight:1.7,resize:"none",fontFamily:"system-ui",minHeight:sec.key==="body"?72:44,padding:0}}
                />
              </div>
            ))}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",overflow:"hidden",background:"#FAFAFA"}}>
            ...
            {/* Insights */}
            <div style={{padding:"12px 14px",borderBottom:`0.5px solid ${C.border}`,overflowY:"auto",flex:"0 0 auto",maxHeight:"55%"}}>
              {/* Disclaimer or personal data */}
              {!hasData?(
                <div style={{padding:"6px 9px",background:"#FFFBEB",border:"0.5px solid #FCD34D",borderRadius:7,marginBottom:10,fontSize:10,color:"#92400E",lineHeight:1.5}}>
                  ⚡ Channel best practices for now — <strong>your insights personalize</strong> as you add metrics
                </div>
              ):(
                <div style={{padding:"6px 9px",background:"#F0FBF6",border:`0.5px solid ${BRAND.green}40`,borderRadius:7,marginBottom:10,fontSize:10,color:BRAND.green,lineHeight:1.5,fontWeight:500}}>
                  ✦ Based on your last {analytics?.videoCount||""} videos
                </div>
              )}
              <div style={{fontSize:9,fontWeight:500,letterSpacing:1,textTransform:"uppercase",color:C.muted,marginBottom:8}}>Top insights · {platform}</div>
              {(hasData?[
                analytics?.topHook&&{text:`Your ${analytics.topHook} hooks outperform everything else`,sub:"Lead with what works for you",color:BRAND.red},
                ...
                <div key={i} style={{display:"flex",gap:7,marginBottom:8}}>
                  <div style={{width:7,height:7,borderRadius:"50%",background:ins.color,flexShrink:0,marginTop:4}}/>
                  <div>
                    <div style={{fontSize:11,color:C.text,lineHeight:1.45}}>{ins.text}</div>
                    <div style={{fontSize:10,color:C.muted,marginTop:1}}>{ins.sub}</div>
                  </div>
                </div>
              ))}
              <div style={{fontSize:9,fontWeight:500,letterSpacing:1,textTransform:"uppercase",color:C.muted,margin:"10px 0 8px"}}>Directions for this section</div>
              {dirs.filter(d=>!focusField||d.field===focusField||["hook","body","cta"].indexOf(focusField)<0).slice(0,2).map((d,i)=>(
                <div key={i} style={{padding:"8px 9px",borderRadius:7,border:`0.5px solid ${C.border}`,marginBottom:6,cursor:"pointer",background:C.surface,transition:"border-color .15s"}}
                  onMouseEnter={e=>e.currentTarget.style.borderColor=BRAND.blue}
                  onMouseLeave={e=>e.currentTarget.style.borderColor=C.border}>
                  <div style={{display:"flex",gap:5,alignItems:"center",marginBottom:3}}>
                    <div style={{width:5,height:5,borderRadius:"50%",background:[BRAND.red,BRAND.yellow,BRAND.blue][i%3]}}/>
                    <div style={{fontSize:11,fontWeight:500,color:C.text}}>{d.label}</div>
                    <div style={{fontSize:9,color:C.muted,marginLeft:"auto",background:C.light,padding:"1px 5px",borderRadius:20}}>{d.field}</div>
                  </div>
                  <div style={{fontSize:10,color:C.muted,lineHeight:1.4,marginBottom:3}}>{d.desc}</div>
                  <div style={{fontSize:9,color:BRAND.blue,fontWeight:500}}>Use as direction →</div>
                </div>
              ))}
            </div>
            ...
            {/* Chat */}
            <div style={{flex:1,display:"flex",flexDirection:"column",padding:"10px 14px",gap:8,overflow:"hidden"}}>
              <div style={{fontSize:9,fontWeight:500,letterSpacing:1,textTransform:"uppercase",color:C.muted}}>Ask anything</div>
              <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:7}}>
                {chatMsgs.map((m,i)=>(
                  <div key={i} style={{
                    padding:"7px 9px",borderRadius:8,fontSize:11,lineHeight:1.55,
                    background:m.role==="ai"?C.surface:BRAND.blue+"18",
                    border:`0.5px solid ${m.role==="ai"?C.border:BRAND.blue+"40"}`,
                    color:C.text,alignSelf:m.role==="user"?"flex-end":"flex-start",
                    maxWidth:"90%",
                  }}>{m.text}</div>
                ))}
                {chatLoading&&<div style={{fontSize:11,color:C.muted,fontStyle:"italic",padding:"4px 0"}}>thinking...</div>}
              </div>
              <div style={{display:"flex",gap:6,flexShrink:0}}>
                <input
                  ...
                  placeholder="Ask about your hook, angle, direction..."
                ...
                <button onClick={sendChat} disabled={!chatInput.trim()||chatLoading}
                  style={{width:28,height:28,borderRadius:"50%",background:chatInput.trim()&&!chatLoading?"#111":C.border,border:"none",cursor:chatInput.trim()&&!chatLoading?"pointer":"not-allowed",color:"#FFF",fontSize:14,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>↑</button>
              </div>
              <div style={{fontSize:9,color:C.muted,textAlign:"center"}}>Asks questions — doesn't write your script</div>
            </div>
          </div>
        </div>

        {/* Footer */}
        <div style={{padding:"9px 18px",borderTop:`0.5px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button onClick={()=>onSave(JSON.stringify(sections))} style={{padding:"7px 16px",background:C.text,color:"#FFF",border:"none",borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:500}}>Save ✓</button>
          </div>
        </div>
      </div>
    </div>
                ...
                return(
                  <div key={v.id} draggable
                    ...
                    <div style={{padding:"10px 12px 0",flex:1,minWidth:0,position:"relative"}}>
                      ...
                      {/* Script bar at card bottom */}
                      {(stage.id==="idea"||stage.id==="production")&&(
                        <div onClick={()=>setScriptCard(v)}
                          style={{marginTop:8,marginLeft:-12,marginRight:-12,padding:"5px 12px",borderTop:`0.5px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",background:"transparent",transition:"background .15s"}}
                          onMouseEnter={e=>e.currentTarget.style.background=BRAND.red+"08"}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <div style={{display:"flex",alignItems:"center",gap:7,flex:1}}>
                            <div style={{height:3,borderRadius:2,background:v.script?BRAND.red:C.border,width:v.script?"100%":"30%",transition:"width .4s,background .3s",flex:1,maxWidth:80}}/>
                            <span style={{fontSize:9,color:v.script?BRAND.red:C.muted,fontWeight:v.script?500:400,whiteSpace:"nowrap"}}>
                              {v.script?`Script · ${(()=>{try{const p=JSON.parse(v.script);return Object.values(p).join(" ").trim().split(/\s+/).filter(Boolean).length}catch(e){return v.script.split(/\s+/).filter(Boolean).length}})()}w`:"Script"}
                            </span>
                          </div>
                        </div>
                      )}
                ...
                return(
                  <div key={v.id} draggable
                    ...
                    <div style={{padding:"10px 11px",flex:1,minWidth:0}}>
                      ...
                      {/* Script bar — bottom of card */}
                      {(stage.id==="idea"||stage.id==="production")&&(
                        <div onClick={()=>setScriptCard(v)}
                          style={{margin:"8px -12px 0",padding:"6px 12px",borderTop:`0.5px solid ${C.border}`,cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"background .15s"}}
                          onMouseEnter={e=>e.currentTarget.style.background=BRAND.red+"08"}
                          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                          <div style={{flex:1,height:3,borderRadius:3,background:v.script?BRAND.red:C.border,overflow:"hidden",position:"relative"}}>
                            {!v.script&&<div style={{position:"absolute",inset:0,background:`repeating-linear-gradient(90deg,${C.border} 0,${C.border} 4px,transparent 4px,transparent 8px)`}}/>}
                          </div>
                          <span style={{fontSize:9,color:v.script?BRAND.red:C.muted,fontWeight:v.script?500:400,flexShrink:0,letterSpacing:.3}}>
                            {v.script?(()=>{try{const p=JSON.parse(v.script);const wc=Object.values(p).join(" ").trim().split(/\s+/).filter(Boolean).length;return wc>0?`Script · ${wc}w`:"Script ·";}catch(e){return"Script ·";}})():"Script"}
                          </span>
                          <AIBoxIcon onClick={(e)=>{e.stopPropagation();setScriptCard(v);}}/>
                        </div>
                      )}
                    </div>
                  </div>
