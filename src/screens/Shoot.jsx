...
function ShootPlanner({card,onSave,onClose}){
  ...
  const addBroll=()=>update(p=>({...p,shots:[...p.shots,{id:uuid(),kind:"visual",section:"broll",sectionLabel:"B-roll",text:"",talentId:null,locationId:null,shotType:"",seconds:""}]}));
        ...
        <div style={{flex:1,display:"grid",gridTemplateColumns:"185px 1fr 250px",overflow:"hidden"}}>
          ...
          <div style={{overflowY:"auto",padding:"11px 14px 18px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9,gap:8}}>
              ...
              <button onClick={addBroll} style={{fontSize:10,padding:"4px 10px",background:C.surface,border:`0.5px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>+ Add b-roll shot</button>
            </div>
            ...
            {plan.shots.map(s=>{
              ...
              const tal=talentOf(s.talentId), loc=locationOf(s.locationId);
              return(
                <div key={s.id} style={{borderLeft:`2px ${s.kind==="visual"?"dashed":"solid"} ${secColor}`,paddingLeft:9,marginBottom:9}}>
