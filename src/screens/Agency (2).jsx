      ...
      {clients.length>0&&(
        <Card pad={0} style={{marginBottom:14,overflow:"hidden"}}>
          ...
          <div style={{padding:"11px 14px"}}>
            <div style={{fontSize:9,fontWeight:600,color:C.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>Clients · tap to open a pipeline</div>
            ...
            {clients.map(client=>{
              ...
              // A pill, not a legend entry — this is the only way into a client
              // who has nothing on the calendar this month.
              return(
                <button key={client.id} onClick={()=>onSelectClient(client)}
                  title={`Open ${client.name}'s pipeline`}
                  style={{display:"inline-flex",alignItems:"center",gap:7,padding:"6px 10px 6px 8px",borderRadius:20,border:`1px solid ${C.border}`,background:C.surface,cursor:"pointer",transition:"all .15s"}}
                  onMouseEnter={e=>{e.currentTarget.style.borderColor=col;e.currentTarget.style.background=col+"0E";}}
                  onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.background=C.surface;}}>
                  <span style={{width:9,height:9,borderRadius:3,background:col,flexShrink:0}}/>
                  <span style={{fontSize:12,color:C.text,fontWeight:500}}>{client.name}</span>
                  <span style={{fontSize:11,color:behind?C.red:C.muted}}>{goal>0?`${pub} / ${goal}`:`${cv.length}`}</span>
                  <span style={{fontSize:11,color:C.muted,marginLeft:1}}>›</span>
                </button>
              );
            })}
            </div>
                      ...
                      <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                        ...
                        {(!isRev&&nCount>0)&&(
                          <button onClick={()=>setNotesVid(v)}
                            style={{fontSize:10,padding:"5px 11px",background:nCount>0?C.purple+"12":C.surface,border:`1px solid ${nCount>0?C.purple+"55":C.border}`,borderRadius:20,cursor:"pointer",color:nCount>0?C.purple:C.text,fontWeight:nCount>0?600:500,display:"inline-flex",alignItems:"center",gap:5,transition:"all .15s"}}
...
function AgencyApp({user,profile,onLogout}){
  ...
  const load=useCallback(async()=>{
    ...
    setClients(cls||[]);
    setVideos((vids||[]).map(r=>({id:r.id,clientId:r.client_id,workspaceId:r.workspace_id,title:r.title,platform:r.platform,stage:r.stage,targetDate:r.target_date,publishDate:r.publish_date,url:r.url||"",hook:r.hook||"",format:r.format||"",cta:r.cta||"",views:r.views||0,likes:r.likes||0,comments:r.comments||0,shares:r.shares||0,saves:r.saves||0,paraTi:r.para_ti,siguiendo:r.siguiendo,busqueda:r.busqueda,metricsAdded:r.metrics_added||false,revision:r.revision||false,videoUrl:r.video_url||"",stageChangedAt:r.stage_changed_at||null,script:r.script||"",shotList:r.shot_list||"",editAdvice:r.edit_advice||"",month:r.month,createdAt:r.created_at})));
    setTargets(tgts||[]);
    setLoading(false);
  },[user.id]);
