...
YOUR FIRST ACTION IS TO SEARCH. Do not answer from memory. Run the web search tool repeatedly — two or three separate searches with different wording — before you write anything. You have no useful knowledge of which ${platform} videos exist; only the search results do.
...
The four queries must attack the subject from different directions, not rephrase each other.`;
  ...
  const runBatch=async(prompt,angle,signal,query)=>{
    try{
      ...
      const d=await readReply(r);
      if(d&&d.error)return{items:[],error:typeof d.error==="string"?d.error:JSON.stringify(d.error).slice(0,160),searched:0};
  ...
  const searchRefs=async(prompt)=>{
    ...
    let firstError=null, totalSearches=0, sawProse=false;
    const take=res=>{
    ...
    const got=take(await runBatch(prompt,SEARCH_ANGLES[0],ctrl.signal,queries[0]));
    setBatchDone(1);
    if(got>0){setLoading(false);setPhase("refs");}

    // The rest fill in behind the grid while the user is already choosing.
    setStreaming(true);
    await Promise.all(SEARCH_ANGLES.slice(1).map(async(angle,idx)=>{
      await new Promise(res=>setTimeout(res,idx*1200));
      take(await runBatch(prompt,angle,ctrl.signal,queries[(idx+1)%queries.length]));
      setBatchDone(b=>b+1);
    }));
