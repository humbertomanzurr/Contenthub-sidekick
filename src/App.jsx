...
function AgencyClientPipeline({client,videos,target,month,workspaceId,businessProfile,onAddVideo,onMoveVideo,onMetrics,onDeleteVideo,onSetTarget,onBack}){
  const[showAdd,setShowAdd]=useState(false);
  const[showGoal,setShowGoal]=useState(false);
  const[metricsVid,setMetricsVid]=useState(null);
  const[overrideVid,setOverrideVid]=useState(null);
  ...
  return(
    <div>
        ...
        {STAGES.map(stage=>{
          ...
          return(
            <div key={stage.id}
              ...
              {cards.map(v=>{
                ...
                return(
                  <div key={v.id} draggable
                    ...
                    <div style={{width:4,background:stage.color,flexShrink:0,borderRadius:"10px 0 0 10px"}}/>
                    <div style={{padding:"11px 12px",flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}><div style={{width:6,height:6,borderRadius:"50%",background:stage.color,flexShrink:0}}/><span style={{fontSize:10,fontWeight:600,color:stage.color}}>{stage.label}</span></div>
                      <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:6,lineHeight:1.3,wordBreak:"break-word"}}>{v.title}</div>
                      <div style={{display:"flex",gap:7,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
                        <span style={{fontSize:10,color:C.muted,background:C.light,padding:"2px 7px",borderRadius:20,border:`1px solid ${C.border}`}}>{PLT[v.platform]||"📱"} {v.platform}</span>
                        {v.targetDate&&<span style={{fontSize:10,color:C.muted}}>📅 {v.targetDate}</span>}
                      </div>
                      {isPub&&!hasMet&&!unlocked&&(<div ref={null} style={{marginBottom:8,padding:"7px 9px",background:"#F8FAFC",borderRadius:7,border:`1px solid ${C.border}`}}><div style={{fontSize:10,fontWeight:600,color:C.muted,marginBottom:5}}>📅 Come back {unlockDate()} for metrics</div><div style={{background:C.border,borderRadius:20,height:4,overflow:"hidden"}}><div style={{width:`${lockPct}%`,height:"100%",background:lockPct>=85?C.green:C.accent,borderRadius:20,transition:"width .3s"}}/></div><div style={{display:"flex",justifyContent:"space-between",marginTop:3}}><span style={{fontSize:9,color:C.muted}}>{daysGone} of 7 days</span><span onClick={()=>setOverrideVid(v)} style={{fontSize:9,color:C.accent,cursor:"pointer",textDecoration:"underline"}}>Already have data?</span></div></div>)}
                      {isPub&&!hasMet&&unlocked&&(<div style={{marginBottom:8,padding:"7px 9px",background:"#DCFCE7",borderRadius:7,border:"1px solid #BBF7D0",display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13}}>✅</span><div><div style={{fontSize:11,fontWeight:700,color:C.green}}>Ready to add metrics!</div><div style={{fontSize:9,color:"#15803D"}}>7 days of data</div></div></div>)}
                      {hasMet&&<div style={{marginBottom:8,padding:"5px 9px",background:"#DCFCE7",borderRadius:7,border:"1px solid #BBF7D0",display:"inline-flex",alignItems:"center",gap:5}}><span>📊</span><span style={{fontSize:10,fontWeight:700,color:C.green}}>Metrics added</span></div>}
                      <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center"}}>
                        {(stage.id==="idea"||stage.id==="production")&&(v.script?<button onClick={()=>setScriptCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",color:C.green,fontWeight:600}}>📄 Script ready</button>:<button onClick={()=>setAiCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#EDE9FE",border:"1px solid #C4B5FD",borderRadius:20,cursor:"pointer",color:"#7C3AED",fontWeight:600}}>✨ Write script with AI</button>)}
                        {stage.id==="production"&&(v.shotList?<button onClick={()=>setShootViewCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:20,cursor:"pointer",color:"#92400E",fontWeight:600}}>🎬 Shot list ready</button>:<button onClick={()=>setShootCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#FEF3C7",border:"1px solid #FCD34D",borderRadius:20,cursor:"pointer",color:"#92400E",fontWeight:600}}>🎥 Plan shoot with AI</button>)}
                        {stage.id==="editing"&&(v.editAdvice?<button onClick={()=>setEditViewCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",color:C.green,fontWeight:600}}>✂️ Edit advice ready</button>:<button onClick={()=>setEditCard(v)} style={{fontSize:11,padding:"5px 12px",background:"#DCFCE7",border:"1px solid #BBF7D0",borderRadius:20,cursor:"pointer",color:C.green,fontWeight:600}}>✨ Editing advice with AI</button>)}
                        {isPub&&!hasMet&&unlocked&&<button onClick={()=>setMetricsVid(v)} style={{fontSize:11,padding:"5px 12px",background:C.green,border:"none",borderRadius:20,cursor:"pointer",color:"#fff",fontWeight:700}}>📊 Add metrics</button>}
                        {!isCf&&<button onClick={()=>setConfirmDel(v.id)} style={{fontSize:11,padding:"4px 8px",background:"none",border:"none",cursor:"pointer",color:C.muted,marginLeft:"auto"}}>🗑</button>}
                        {isCf&&<div style={{display:"flex",gap:4,alignItems:"center",marginLeft:"auto"}}><span style={{fontSize:10,color:C.red,fontWeight:700}}>Delete?</span><button onClick={()=>{onDeleteVideo(v.id);setConfirmDel(null);}} style={{fontSize:10,padding:"2px 7px",background:C.red,border:"none",borderRadius:20,cursor:"pointer",color:"#fff",fontWeight:700}}>Yes</button><button onClick={()=>setConfirmDel(null)} style={{fontSize:10,padding:"2px 7px",background:C.light,border:`1px solid ${C.border}`,borderRadius:20,cursor:"pointer",color:C.text}}>No</button></div>}
                      </div>
                    </div>
                  </div>
              ...
              {stage.id==="idea"&&goal===0&&cards.length===0&&<div style={{textAlign:"center",padding:"16px 8px",color:C.muted,fontSize:11,border:`1px dashed ${C.border}`,borderRadius:8,background:C.bg,lineHeight:1.5}}>Set a goal above<br/>to fill your pipeline</div>}
              {stage.id==="idea"&&[...Array(emptyCount)].map((_,i)=><div key={`e${i}`} onClick={()=>setShowAdd(true)} style={{background:C.surface,borderRadius:8,padding:12,marginBottom:7,border:`1px dashed ${C.border}`,cursor:"pointer",textAlign:"center"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=C.accent;e.currentTarget.style.background="#EFF6FF";}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.background=C.surface;}}><div style={{fontSize:16,marginBottom:3}}>+</div><div style={{fontSize:10,fontWeight:700,color:C.muted}}>Fill me out</div></div>)}
              {cards.length===0&&stage.id!=="idea"&&<div style={{textAlign:"center",padding:"18px 6px",color:C.muted,fontSize:10,border:`1px dashed ${C.border}`,borderRadius:7,background:C.bg}}>Nothing here yet</div>}
            </div>
          );
        })}
      ...
      {overrideVid&&(<div style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,fontFamily:"system-ui"}}><div style={{background:C.surface,borderRadius:16,border:`1px solid ${C.border}`,boxShadow:"0 8px 32px rgba(0,0,0,.2)",width:"min(380px,95vw)",padding:26}}><div style={{fontSize:15,fontWeight:800,color:C.text,marginBottom:6}}>📊 Add metrics early</div><div style={{fontSize:13,color:C.muted,marginBottom:16,lineHeight:1.5}}>The 7-day wait gives your video time to collect real data. But if you already have the numbers, go ahead.</div><div style={{padding:12,background:"#FFFBEB",borderRadius:8,border:`1px solid ${C.amber}40`,marginBottom:18}}><div style={{fontSize:12,color:C.amber,fontWeight:600}}>{overrideVid.title}</div></div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><Btn onClick={()=>setOverrideVid(null)}>Cancel</Btn><button onClick={()=>{setMetricsVid(overrideVid);setOverrideVid(null);}} style={{padding:"8px 16px",background:C.text,color:"#FFF",border:"none",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:700}}>Add metrics now →</button></div></div></div>)}
      {metricsVid&&<MetricsModal video={metricsVid} onSave={m=>{onMetrics(metricsVid.id,m);setMetricsVid(null);}} onClose={()=>setMetricsVid(null)}/>}
      {scriptCard&&<ScriptViewer card={scriptCard} businessProfile={businessProfile} onSave={s=>{onMetrics&&null;setScriptCard(null);}} onRewrite={()=>{setAiCard(scriptCard);setScriptCard(null);}} onClose={()=>setScriptCard(null)}/>}
      {aiCard&&(()=>{const wm=[];const analytics={topHook:null,topFormat:null};return <AIScriptChat card={aiCard} businessProfile={businessProfile} analytics={analytics} onSaveScript={s=>{setAiCard(null);}} onClose={()=>setAiCard(null)}/>;})()}
      {shootCard&&<ShootDirector card={shootCard} businessProfile={businessProfile} onSaveShotList={sl=>{setShootCard(null);}} onClose={()=>setShootCard(null)}/>}
      {shootViewCard&&<ShotListViewer card={shootViewCard} businessProfile={businessProfile} onSave={sl=>{setShootViewCard(null);}} onReplan={()=>{setShootCard(shootViewCard);setShootViewCard(null);}} onClose={()=>setShootViewCard(null)}/>}
      {editCard&&<EditAdviceGenerator card={editCard} businessProfile={businessProfile} analytics={{topHook:null,topFormat:null}} onSave={advice=>{setEditCard(null);}} onClose={()=>setEditCard(null)}/>}
      {editViewCard&&<EditAdviceViewer card={editViewCard} onRegenerate={()=>{setEditCard(editViewCard);setEditViewCard(null);}} onClose={()=>setEditViewCard(null)}/>}
    </div>
  );
}
...
function AgencyApp({user,profile,onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:20}}>
        {!selectedClient
          ?<AgencyDashboard clients={clients} videos={videos} targets={targets} month={month} onMonthChange={setMonth} onSelectClient={setSelectedClient} onAddClient={addClient} onSetTarget={setTarget} businessProfile={profile?.business_profile}/>
          :<AgencyClientPipeline client={selectedClient} videos={clientVids} target={clientTarget} month={month} workspaceId={wsId} businessProfile={profile?.business_profile} onAddVideo={addVideo} onMoveVideo={moveVideo} onMetrics={saveMetrics} onDeleteVideo={deleteVideo} onSetTarget={setTarget} onBack={()=>setSelectedClient(null)}/>
        }
      </div>
    </div>
  );
