...
function CampaignCreator({userId,businessProfile,videos,onSendToPipeline}){
  ...
  const buildSearchSystem=(angle)=>`You are a content strategist finding real, existing ${platform} videos to use as references.
...
Search the web for real ${platform} videos related to the campaign topic. For this pass, focus on ${angle}.
...
- If you cannot find the real URL for a video, leave that video out entirely. Returning fewer real results is the correct behaviour.
...
Return up to 25 items. JSON only.`;

  const runBatch=async(prompt,angle,signal)=>{
    try{
      const r=await fetch("/api/chat",{
        method:"POST",signal,headers:{"Content-Type":"application/json"},
        body:JSON.stringify({messages:[{role:"user",content:`Campaign: ${prompt}`}],systemPrompt:buildSearchSystem(angle),useWebSearch:true})
      });
      const d=await r.json();
  ...
  const searchRefs=async(prompt)=>{
    ...
    setPhase("searching");
    setLoading(true);
    setRefs([]);setSelected([]);setDropped(0);setBatchDone(0);
    ...
    // First pass decides how fast anything appears on screen.
    const got=absorb(await runBatch(prompt,SEARCH_ANGLES[0],ctrl.signal));
    setBatchDone(1);
    if(got>0){setLoading(false);setPhase("refs");}

    // The rest fill in behind the grid while the user is already choosing.
    setStreaming(true);
    await Promise.all(SEARCH_ANGLES.slice(1).map(async angle=>{
      absorb(await runBatch(prompt,angle,ctrl.signal));
      setBatchDone(b=>b+1);
    }));
...
${JSON.stringify(selectedItems,null,2)}`}],
  ...
  const reset=()=>{setPhase("start");setInput("");setContext("");setRefs([]);setSelected([]);setCampaign(null);setStreaming(false);setBatchDone(0);setDropped(0);};
  ...
  if(phase==="start")return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:500,padding:"48px 24px",textAlign:"center"}}>
      <div style={{fontSize:28,fontWeight:500,color:C.text,marginBottom:28,letterSpacing:-0.5}}>What are you creating this month?</div>

      {/* Platform — results are restricted to whichever one is picked */}
      <div style={{marginBottom:18,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
        <div style={{fontSize:10,fontWeight:500,letterSpacing:1,textTransform:"uppercase",color:C.muted}}>Find references on</div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center"}}>
          {CAMPAIGN_PLATFORMS.map(p=>{
            const on=platform===p;
            return(
              <button key={p} onClick={()=>setPlatform(p)}
                style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:on?600:400,color:on?C.text:C.muted,background:on?C.surface:"transparent",border:`1px solid ${on?C.text:C.border}`,transition:"all .15s"}}>
      ...
      <div style={{width:"min(580px,100%)",position:"relative",marginBottom:20}}>
        <textarea
          ...
          placeholder={`Describe the campaign — what you're launching, who it's for, what you want it to do. (Enter to search ${platform})`}
    ...
    <div>
      ...
      {refs.length===0&&!streaming&&(
        <div style={{textAlign:"center",padding:"56px 24px",border:`1px dashed ${C.border}`,borderRadius:12,background:C.light}}>
          ...
          <div style={{fontSize:15,fontWeight:700,color:C.text,marginBottom:6}}>No {platform} videos came back with a working link</div>
          <div style={{fontSize:13,color:C.muted,lineHeight:1.6,maxWidth:400,margin:"0 auto 18px"}}>Try naming the topic more specifically, or pick a different platform — some have far more that's publicly searchable than others.</div>
