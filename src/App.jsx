...
function CreatorApp({user,profile,onLogout}){
  ...
  // One-time tour check — runs once on mount only
  useEffect(()=>{
    const key=`sk_tour_done_${user.id}`;
  ...
  const finishTour=()=>{
    localStorage.setItem(`sk_tour_done_${user.id}`,"1");
    setTourStep(0);
  };
  const nextTourStep=()=>{
    if(tourStep>=5){finishTour();}
    else{setTourStep(s=>s+1);}
  };

  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      {tourStep===1&&refGoal.current&&<TourBubble step={1} total={5} text="👋 Welcome! First things first — set how many videos you want to publish this month. Click the goal button to set it." action="Set my goal →" onAction={()=>setShowGoalModal(true)} onSkip={finishTour} position={{ref:refGoal,side:"bottom"}}/>}
      {tourStep===2&&refAdd.current&&<TourBubble step={2} total={5} text="➕ Now add your first video idea. Click the + Add video button, give it a title, pick a platform, and set a target date." action="Got it →" onAction={()=>{}} onSkip={finishTour} position={{ref:refAdd,side:"bottom"}}/>}
      {tourStep===3&&refStages.current&&<TourBubble step={3} total={5} text="🚀 Move your card through the stages as you work on it. Drag it from Idea → Production → Editing. Try moving it now — the next step unlocks automatically." action="Got it →" onAction={()=>nextTourStep()} onSkip={finishTour} position={{ref:refStages,side:"top"}}/>}
      {tourStep===4&&refPublished.current&&<TourBubble step={4} total={5} text="📱 When your video is live on TikTok or Instagram, drag the card to Published. The app doesn't post for you — you publish it on social media, then drag it here to mark it done." action="Got it →" onAction={()=>nextTourStep()} onSkip={finishTour} position={{ref:refPublished,side:"top"}}/>}
      ...
      {showGoalModal&&<GoalModal month={month} current={goals[month]||0} onSave={async v=>{await setGoal(month,v);setShowGoalModal(false);if(tourStep===1)nextTourStep();}} onClose={()=>setShowGoalModal(false)}/>}
      ...
      <div style={{flex:1,overflowY:"auto",padding:20}}>
        {page==="dashboard"&&<CreatorDashboard videos={videos} goals={goals} month={month} onMonthChange={setMonth} onSetGoal={setGoal} onAddVideo={(v)=>{addV(v);if(tourStep===2)nextTourStep();}} onMoveVideo={(id,st)=>{moveV(id,st);if(tourStep===3)nextTourStep();if(tourStep===4&&st==="published")nextTourStep();}} onMetrics={saveM} onDeleteVideo={delV} userId={user.id} refGoal={refGoal} refAdd={refAdd} refStages={refStages} refPublished={refPublished} refLock={refLock} tourStep={tourStep} onTourNext={nextTourStep}/>}
