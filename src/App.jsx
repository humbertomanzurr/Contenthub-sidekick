...
const getWorkspaceMember = async (userId) => {
  try {
    ...
    if(!r.ok){console.error("getWorkspaceMember failed:",r.status,await r.text());return null;}
  ...
  } catch(e){console.error("getWorkspaceMember error:",e);return null;}
};
              ...
              {!showSendBack?(
                <div style={{display:"flex",gap:7}}>
                  <button onClick={()=>setShowSendBack(true)}
                    ...
                    <span style={{width:6,height:6,borderRadius:"50%",background:"#F97316"}}/>Send back
                  </button>
...
function AgencyApp({user,profile,onLogout}){
  ...
  const load=useCallback(async()=>{
    ...
    setClients(cls||[]);
    setVideos((vids||[]).map(r=>({id:r.id,clientId:r.client_id,workspaceId:r.workspace_id,title:r.title,platform:r.platform,stage:r.stage,targetDate:r.target_date,publishDate:r.publish_date,url:r.url||"",hook:r.hook||"",format:r.format||"",cta:r.cta||"",views:r.views||0,likes:r.likes||0,comments:r.comments||0,shares:r.shares||0,saves:r.saves||0,paraTi:r.para_ti,siguiendo:r.siguiendo,busqueda:r.busqueda,metricsAdded:r.metrics_added||false,revision:r.revision||false,videoUrl:r.video_url||"",script:r.script||"",shotList:r.shot_list||"",editAdvice:r.edit_advice||"",month:r.month,createdAt:r.created_at})));
    setTargets(tgts||[]);
    setLoading(false);
  },[user.id]);
  ...
  const moveVideo=useCallback(async(id,st,pd,opts)=>{
    ...
    setVideos(prev=>prev.map(v=>v.id===id?{...v,stage:st,publishDate:pubD||v.publishDate,
