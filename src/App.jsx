...
Return up to 25 items. JSON only.`;
  ...
  const runBatch=async(prompt,angle,signal)=>{
    try{
      const r=await fetch("/api/chat",{
        ...
        body:JSON.stringify({
          messages:[{role:"user",content:`Campaign: ${prompt}`}],
          systemPrompt:buildSearchSystem(angle),
          useWebSearch:true,
      ...
      const d=await r.json();
      if(d&&d.error)return{items:[],error:typeof d.error==="string"?d.error:"Search failed",searched:0};
      ...
      if(a<0||b<0)return{items:[],error:null,searched,noJson:true};
      try{
        const parsed=JSON.parse(clean.slice(a,b+1));
        return{items:Array.isArray(parsed)?parsed:[],error:null,searched};
  ...
  const searchRefs=async(prompt)=>{
    ...
    setPhase("searching");
    setLoading(true);
    setRefs([]);setSelected([]);setDropped(0);setBatchDone(0);setSearchError(null);setSearchCalls(0);
    ...
    let firstError=null, totalSearches=0;
    const take=res=>{
    ...
    const got=take(await runBatch(prompt,SEARCH_ANGLES[0],ctrl.signal));
    setBatchDone(1);
    ...
    await Promise.all(SEARCH_ANGLES.slice(1).map(async angle=>{
      take(await runBatch(prompt,angle,ctrl.signal));
      setBatchDone(b=>b+1);
    }));
        ...
        <div style={{textAlign:"center",padding:"56px 24px",border:`1px dashed ${C.border}`,borderRadius:12,background:C.light}}>
          ...
          <div style={{fontSize:13,color:C.muted,lineHeight:1.6,maxWidth:420,margin:"0 auto 14px"}}>
            {searchError
              ...
              :searchCalls===0
                ?`The search ran but never queried the web. Try a topic with more specific wording.`
