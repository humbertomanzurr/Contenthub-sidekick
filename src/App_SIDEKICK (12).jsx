...
// ── CREATOR DASHBOARD ─────────────────────────────────────────────────────────
function CreatorDashboard({videos,goals,month,onMonthChange,onSetGoal,onAddVideo,onMoveVideo,onMetrics,onDeleteVideo,userId,refGoal,refAdd,refStages,refPublished,refLock,tourStep,showAddFromTour,onAddFromTourDone,onTourNext}){
...
function CreatorApp({user,profile,onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      {tourStep===1&&<TourBubble step={1} total={6} emoji={TOUR_STEPS[0].emoji} title={TOUR_STEPS[0].title} body={TOUR_STEPS[0].body} action={TOUR_STEPS[0].action} onAction={tourAction1} onSkip={finishTour} targetRef={refGoal} side="bottom"/>}
      {tourStep===2&&<TourBubble step={2} total={6} emoji={TOUR_STEPS[1].emoji} title={TOUR_STEPS[1].title} body={TOUR_STEPS[1].body} action={TOUR_STEPS[1].action} onAction={tourAction2} onSkip={finishTour} targetRef={refAdd} side="bottom"/>}
      {tourStep===3&&<TourBubble step={3} total={6} emoji={TOUR_STEPS[2].emoji} title={TOUR_STEPS[2].title} body={TOUR_STEPS[2].body} action={TOUR_STEPS[2].action} onAction={tourAction3} onSkip={finishTour} targetRef={refStages} side="top"/>}
      {tourStep===4&&<TourBubble step={4} total={6} emoji={TOUR_STEPS[3].emoji} title={TOUR_STEPS[3].title} body={TOUR_STEPS[3].body} action={TOUR_STEPS[3].action} onAction={tourAction4} onSkip={finishTour} targetRef={refPublished} side="top"/>}
      {tourStep===5&&<TourBubble step={5} total={6} emoji={TOUR_STEPS[4].emoji} title={TOUR_STEPS[4].title} body={TOUR_STEPS[4].body} action={TOUR_STEPS[4].action} onAction={tourAction5} onSkip={finishTour} targetRef={refLock} side="left"/>}
      {tourStep===6&&<TourBubble step={6} total={6} emoji={TOUR_STEPS[5].emoji} title={TOUR_STEPS[5].title} body={TOUR_STEPS[5].body} action={TOUR_STEPS[5].action} onAction={tourAction6} onSkip={finishTour} targetRef={refLock} side="left"/>}
      ...
      <div style={{flex:1,overflowY:"auto",padding:20}}>
        {page==="dashboard"&&<CreatorDashboard videos={videos} goals={goals} month={month} onMonthChange={setMonth} onSetGoal={setGoal} onAddVideo={(v)=>{addV(v);if(tourNext!==null){showStep(tourNext);setTourNext(null);}}} onMoveVideo={(id,st)=>{moveV(id,st);if(tourStep===4&&st==="published")showStep(5);}} onMetrics={saveM} onDeleteVideo={delV} userId={user.id} refGoal={refGoal} refAdd={refAdd} refStages={refStages} refPublished={refPublished} refLock={refLock} tourStep={tourStep} showAddFromTour={showAddFromTour} onAddFromTourDone={()=>{setShowAddFromTour(false);}} onTourNext={()=>{if(tourNext!==null){showStep(tourNext);setTourNext(null);}}}/>}
