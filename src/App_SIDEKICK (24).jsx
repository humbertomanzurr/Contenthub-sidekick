...
// ── AI SCRIPT ASSISTANT ───────────────────────────────────────────────────────
function AIScriptChat({card, businessProfile, analytics, onClose}){
        ...
        <div style={{flex:1,overflowY:"auto",padding:16,display:"flex",flexDirection:"column",gap:12}}>
          ...
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start"}}>
              ...
              {m.role==="assistant"&&(
                <button onClick={()=>copyText(m.content)} style={{fontSize:10,color:C.muted,background:"none",border:"none",cursor:"pointer",marginTop:4,marginLeft:2,padding:0}}>
                  Copy ↗
                </button>
              )}
            </div>
...
// ── CREATOR DASHBOARD ─────────────────────────────────────────────────────────
function CreatorDashboard({videos,goals,month,onMonthChange,onSetGoal,onAddVideo,onMoveVideo,onMetrics,onDeleteVideo,userId,refGoal,refAdd,refStages,refPublished,refLock,tourStep,showAddFromTour,onAddFromTourDone,onTourNext,businessProfile}){
  ...
  return(
    <div>
                ...
                return(
                  <div key={v.id} draggable
                    ...
                    <div style={{padding:"11px 12px",flex:1,minWidth:0}}>
                      ...
                      {(stage.id==="idea"||stage.id==="production")&&<button onClick={()=>setAiCard(v)} style={{fontSize:10,padding:"4px 10px",background:"#EDE9FE",border:"1px solid #C4B5FD",borderRadius:20,cursor:"pointer",color:"#7C3AED",fontWeight:600}}>✨ Write script</button>}
      ...
      {aiCard&&(()=>{
        ...
        return <AIScriptChat card={aiCard} businessProfile={businessProfile} analytics={analytics} onClose={()=>setAiCard(null)}/>;
      })()}
    </div>
  );
}
    ...
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{width:"min(480px,100%)",background:C.surface,borderRadius:20,border:`1px solid ${C.border}`,boxShadow:shMd,padding:"40px 36px",textAlign:"center"}}>
        ...
        <div style={{fontSize:14,color:C.muted,lineHeight:1.7,marginBottom:12,maxWidth:360,margin:"0 auto 12px"}}>
          We need 60 seconds of context about your business. This powers everything — the pipeline, the analytics, and the AI script assistant that writes content in your brand voice.
        </div>
