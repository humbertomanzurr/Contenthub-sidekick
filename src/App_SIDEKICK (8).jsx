...
// ── CONTEXTUAL TOUR ──────────────────────────────────────────────────────────
function TourBubble({text,action,onAction,onSkip,position,step,total}){
  ...
  useEffect(()=>{
    ...
    const update=()=>{
      const r=position.ref.current.getBoundingClientRect();
      setRect(r);setVisible(true);
    };
    update();
  ...
  const GAP=12;
  let bubbleStyle={position:"fixed",zIndex:10000,width:260,background:C.text,color:"#FFF",borderRadius:14,padding:"14px 16px",boxShadow:"0 8px 32px rgba(0,0,0,.3)",fontFamily:"system-ui,sans-serif"};

  if(side==="bottom"){
    bubbleStyle.top=rect.bottom+GAP;
    bubbleStyle.left=Math.min(Math.max(rect.left+rect.width/2-130,10),window.innerWidth-270);
  }else if(side==="top"){
    bubbleStyle.bottom=window.innerHeight-rect.top+GAP;
    bubbleStyle.left=Math.min(Math.max(rect.left+rect.width/2-130,10),window.innerWidth-270);
  }else if(side==="left"){
    bubbleStyle.top=rect.top+rect.height/2-60;
  ...
  let arrowStyle={};
  if(side==="bottom") arrowStyle={...arrowBase,top:-7,left:"50%",transform:"translateX(-50%)",borderLeft:"7px solid transparent",borderRight:"7px solid transparent",borderBottom:`7px solid ${C.text}`};
  if(side==="top") arrowStyle={...arrowBase,bottom:-7,left:"50%",transform:"translateX(-50%)",borderLeft:"7px solid transparent",borderRight:"7px solid transparent",borderTop:`7px solid ${C.text}`};
  if(side==="right") arrowStyle={...arrowBase,left:-7,top:"50%",transform:"translateY(-50%)",borderTop:"7px solid transparent",borderBottom:"7px solid transparent",borderRight:`7px solid ${C.text}`};
  if(side==="left") arrowStyle={...arrowBase,right:-7,top:"50%",transform:"translateY(-50%)",borderTop:"7px solid transparent",borderBottom:"7px solid transparent",borderLeft:`7px solid ${C.text}`};

  // Spotlight highlight on target
  const spotStyle={position:"fixed",top:rect.top-6,left:rect.left-6,width:rect.width+12,height:rect.height+12,borderRadius:10,boxShadow:`0 0 0 9999px rgba(15,23,42,.7)`,zIndex:9999,pointerEvents:"none",border:`2px solid ${C.accent}`,transition:"all .3s"};
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <div style={{display:"flex",gap:4}}>
            {Array.from({length:total}).map((_,i)=>(
              <div key={i} style={{width:i===step-1?14:5,height:5,borderRadius:20,background:i<step?"#FFF":"rgba(255,255,255,.3)",transition:"all .2s"}}/>
            ))}
          </div>
          <button onClick={onSkip} style={{background:"none",border:"none",color:"rgba(255,255,255,.5)",fontSize:11,cursor:"pointer",padding:0}}>Skip tour</button>
        </div>
        <div style={{fontSize:13,lineHeight:1.6,marginBottom:12,color:"rgba(255,255,255,.9)"}}>{text}</div>
        <button onClick={onAction} style={{width:"100%",padding:"8px 0",background:"rgba(255,255,255,.15)",border:"1px solid rgba(255,255,255,.25)",borderRadius:8,color:"#FFF",fontSize:13,fontWeight:700,cursor:"pointer"}}>{action}</button>
      </div>
    </>
  );
}
...
// ── CREATOR DASHBOARD ─────────────────────────────────────────────────────────
function CreatorDashboard({videos,goals,month,onMonthChange,onSetGoal,onAddVideo,onMoveVideo,onMetrics,onDeleteVideo,userId,refGoal,refAdd,refStages,refPublished,refLock,tourStep,onTourNext}){
    ...
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:7}}>
          ...
          <span ref={refAdd}><Btn primary onClick={()=>{setShowAdd(true);if(tourStep===2)onTourNext&&onTourNext();}}>+ Add video</Btn></span>
        </div>
      </div>
                ...
                return(
                  <div key={v.id} draggable
                    ...
                    {isPub&&!hasMet&&!unlocked&&(
                      <div ref={refLock} style={{marginBottom:8}}>
...
function CreatorApp({user,profile,onLogout}){
  ...
  const[tourStep,setTourStep]=useState(0); // 0=off, 1-5=active
  ...
  const load=useCallback(async()=>{
    ...
    if(isNew) setTimeout(()=>setTourStep(1), 800);
    setLoading(false);
  },[user.id]);

  useEffect(()=>{load();},[load]);
  useEffect(()=>{const p=setInterval(load,30000);return()=>clearInterval(p);},[load]);
  ...
  const addV=useCallback(async v=>{
    ...
    sbInsert("creator_videos",{id:v.id,user_id:user.id,title:v.title,platform:v.platform||"TikTok",stage:"idea",target_date:v.targetDate||null,publish_date:null,url:"",hook:"",format:"",cta:"",views:0,likes:0,comments:0,shares:0,saves:0,para_ti:null,siguiendo:null,busqueda:null,pauta:0,metrics_added:false,month:v.month||curMonth(),created_at:new Date().toISOString()});
  },[user.id]);
  const moveV=useCallback(async(id,st)=>{
    // Optimistic update — change UI instantly
    setVideos(prev=>prev.map(v=>v.id===id?{...v,stage:st,publishDate:st==="published"?new Date().toISOString().slice(0,10):v.publishDate}:v));
    // Sync to database in background
    const up={stage:st};if(st==="published")up.publish_date=new Date().toISOString().slice(0,10);
    sbUpdate("creator_videos","id",id,up);
  },[]);
  const saveM=useCallback(async(id,m)=>{
    // Optimistic metrics update
    setVideos(prev=>prev.map(v=>v.id===id?{...v,url:m.url||"",hook:m.hook||"",format:m.format||"",cta:m.cta||"",views:m.views||0,likes:m.likes||0,comments:m.comments||0,shares:m.shares||0,saves:m.saves||0,paraTi:m.paraTi||null,siguiendo:m.siguiendo||null,busqueda:m.busqueda||null,pauta:m.pauta||0,metricsAdded:true}:v));
    sbUpdate("creator_videos","id",id,{url:m.url||"",hook:m.hook||"",format:m.format||"",cta:m.cta||"",views:m.views||0,likes:m.likes||0,comments:m.comments||0,shares:m.shares||0,saves:m.saves||0,para_ti:m.paraTi||null,siguiendo:m.siguiendo||null,busqueda:m.busqueda||null,pauta:m.pauta||0,metrics_added:true});
  },[]);
  const delV=useCallback(async id=>{
    setVideos(prev=>prev.filter(v=>v.id!==id));
    sbDelete("creator_videos","id",id);
  },[]);
  ...
  const finishTour=()=>{localStorage.setItem(`sk_tour_done_${user.id}`,"1");setTourStep(0);};
  const nextTourStep=()=>{if(tourStep>=5){finishTour();}else{setTourStep(s=>s+1);}};

  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      {tourStep===1&&refGoal.current&&<TourBubble step={1} total={5} text="👋 Welcome! First things first — set how many videos you want to publish this month. This is your goal. Everything tracks from here." action="Set my goal →" onAction={()=>{setShowGoalModal(true);nextTourStep();}} onSkip={finishTour} position={{ref:refGoal,side:"bottom"}}/>}
      {tourStep===2&&refAdd.current&&<TourBubble step={2} total={5} text="➕ Got a video idea? Click here to add it. Just give it a title, pick your platform, and set a date. Takes 10 seconds." action="Add my first video →" onAction={()=>nextTourStep()} onSkip={finishTour} position={{ref:refAdd,side:"bottom"}}/>}
      {tourStep===3&&refStages.current&&<TourBubble step={3} total={5} text="🚀 These are your pipeline stages. As you work on your video, drag the card from Idea → Production → Editing. It keeps you organized and accountable." action="Got it →" onAction={()=>nextTourStep()} onSkip={finishTour} position={{ref:refStages,side:"top"}}/>}
      {tourStep===4&&refPublished.current&&<TourBubble step={4} total={5} text="📱 When your video is live on TikTok or Instagram, drag the card here. The app doesn't post for you — you publish it yourself, then mark it done here." action="Makes sense →" onAction={()=>nextTourStep()} onSkip={finishTour} position={{ref:refPublished,side:"top"}}/>}
      {tourStep===5&&refLock.current&&<TourBubble step={5} total={5} text="📊 After publishing, your card locks for 7 days. Why? Because views and engagement keep growing for a week. Come back on day 7 and add your real metrics. That's when the analytics get good." action="Let's go! 🚀" onAction={finishTour} onSkip={finishTour} position={{ref:refLock,side:"left"}}/>}
      {showGoalModal&&<GoalModal month={month} current={goals[month]||0} onSave={async v=>{await setGoal(month,v);setShowGoalModal(false);}} onClose={()=>setShowGoalModal(false)}/>}
      ...
      <div style={{flex:1,overflowY:"auto",padding:20}}>
        {page==="dashboard"&&<CreatorDashboard videos={videos} goals={goals} month={month} onMonthChange={setMonth} onSetGoal={setGoal} onAddVideo={addV} onMoveVideo={moveV} onMetrics={saveM} onDeleteVideo={delV} userId={user.id} refGoal={refGoal} refAdd={refAdd} refStages={refStages} refPublished={refPublished} refLock={refLock} tourStep={tourStep} onTourNext={nextTourStep}/>}
